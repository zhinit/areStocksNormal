...
ui_interval = col2.selectbox(
    "Pick a time interval (nonoverlapping jumps)", intervals, index=1
)
...
if len(data) > 30:
    ...
    valid = False
    st.write(
        "There are less than 30 data points. This is insufficient to complete the analysis."
    )
...
if valid:
    ...
    if skew < -0.5:
        skew_msg = (
            "the distribution has a long left tail because people were panic selling!"
        ...
        skew_msg = (
            "the distribution has a long right tail because people are euphoric buying!"
            "This indicates the returns do not follow a normal distribution"
        )

    ex_kurt = np.power((data["return"] - mu) / std, 4).sum() * n * (n + 1) / (
        (n - 1) * (n - 2) * (n - 3)
    ) - 3 * (n - 1) ** 2 / ((n - 2) * (n - 3))
    if ex_kurt < -0.5:
        kurt_msg = (
            "the distribution has light tails because this stock is boring and doesnt move much. "
            "This indicates the returns do not follow a normal distribution"
        )
    elif ex_kurt < 0.5:
        kurt_msg = (
            'the distribution has "normal" weight in the tails '
            "This indicates the returns follow a normal distribution"
        )
    else:
        kurt_msg = (
            "the distribution has heavy tails because the stock has extreme price moves. "
    ...
    st.text(
        "Note that a bell curve should be symmetric.\n"
    ...
    st.text("")
    st.text(
        "Note that a bell curve should have a medium amount of weight in its tails.\n"
    ...
    perfectly_normal_data = mu + std * np.sqrt(2) * erfinv(
        2 * (np.arange(1, n + 1) - 0.5) / n - 1
    )
    ...
    def get_p_msg(p_value):
        if p_value < 0.01:
            return (
                "We have strong statistical evidence to reject the idea that "
                "the stock returns come from a normal distribution."
            )
        elif p_value < 0.05:
            return (
                "We have some statistical evidence to reject the idea that "
                "data come from a normal distribution."
            )
        else:
            return (
                "We have statistical evidence to fail to reject the idea that "
